...
function ThemeToggle() {
  ...
  return (
    <button
      ...
      <span className="opacity-70 group-hover:opacity-100 transition">{isDark ? "Light" : "Dark"}</span>
    </button>
  );
}
...
export default function Hero() {
  ...
  return (
    <section className="relative min-h-[90svh] overflow-hidden">
      ...
      {/* Expand content size TODO */}
      <div className="relative z-10 mx-auto flex max-w-7xl flex-col items-center gap-8 px-6 pt-24 text-center sm:pt-32">
        <ThemeToggle />

        <h1 className="mt-4 font-extrabold tracking-tight">
        ...
        <div className="mt-2 flex flex-wrap items-center justify-center gap-3 text-sm text-zinc-600 dark:text-zinc-400">
      ...
      {/* Parallax isn't dead!!!  Reinstall animator*/}
      <ParallaxBits />
    </section>
  );
}
...
function Dot() {
  return <span className="mx-1 inline-block h-1 w-1 rounded-full bg-zinc-400 dark:bg-zinc-600" />;
}
...
function HoverLink({ href, children }) {
  return (
    <a
      href={href}
      className="relative px-1 transition hover:brightness-110"
    >
      <span className="relative z-10">{children}</span>
      <span className="absolute -bottom-0.5 left-0 right-0 h-[2px] origin-left scale-x-0 rounded bg-gradient-to-r from-fuchsia-400 via-sky-400 to-emerald-400 transition will-change-transform group-hover:scale-x-100 peer-hover:scale-x-100 hover:scale-x-100" />
    </a>
  );
}
...
function GridOverlay() {
  return (
    <div aria-hidden className="pointer-events-none absolute inset-0 -z-10 opacity-[0.12] dark:opacity-[0.18]">
